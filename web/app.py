...
uploaded_file = st.file_uploader(
    "选择 FITS 文件或普通图像（PNG/JPG）",
    type=["fits", "fz", "png", "jpg", "jpeg"]
)
...
if uploaded_file is not None:
    ...
    if data is not None:
        ...
        if st.button("开始检测", type="primary"):
            with st.spinner("检测中..."):
                ...
                use_method = method.split(" ")[0]
                
                if use_method == "unet":
                    model_path = os.path.join(os.path.dirname(__file__), "..", "models", "fmo_unet.pth")
                    if os.path.exists(model_path):
                        detector = StripeDetector(method="unet", model_path=model_path)
                    else:
                        st.warning("找不到 U-Net 模型，使用传统方法")
                        detector = StripeDetector(method="traditional")
                else:
                    detector = StripeDetector(method="traditional")
                
                candidates = detector.detect_traditional(
                    data,
                    n_sigma=n_sigma,
    ...
    st.markdown("- **FITS** (`.fits`, `.fits.fz`)：天文标准格式，可直接从 ZTF、其他望远镜数据获取")
